...
/**
 * Compares the behavior of the educational Mini RxJava implementation
 * with the real RxJava library in key scenarios.
 *
 * <p>This class focuses on semantic behavior rather than performance.
 * The goal is to verify that both implementations produce the same:
 * <ul>
 *     <li>items,</li>
 ...
 * </ul>
 */
public class RxJavaComparisonTest {

    /**
     * Verifies that a simple source produces the same items
     * and completion behavior in both implementations.
     */
    @Test
    ...
    /**
     * Verifies that the flatMap operator behaves the same in both implementations
     * for a simple synchronous scenario.
     */
    @Test
     ...
     * @return collected comparison data
     */
    private <T> ComparisonResult<T> collectFromRxJava(io.reactivex.rxjava3.core.Observable<T> observable) {
        List<T> items = new ArrayList<>();
        AtomicBoolean completed = new AtomicBoolean(false);
        AtomicReference<Throwable> error = new AtomicReference<>();
     ...
     * @param <T> the item type
     */
    private <T> void assertComparisonEquals(ComparisonResult<T> miniRxResult, ComparisonResult<T> rxJavaResult) {

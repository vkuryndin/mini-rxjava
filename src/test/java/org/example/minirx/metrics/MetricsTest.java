...
/**
 * Collects basic functional and timing metrics for the educational Mini RxJava implementation and
 * compares them with the real RxJava library.
 *
 * <p>This class is intentionally focused on:
 *
 * <ul>
 *   <li>functional correctness,
 *   <li>completion correctness,
 *   <li>error propagation correctness,
 *   <li>basic scheduler behavior,
...
class MetricsTest {
  ...
  /**
   * Collects metrics for a synchronous map + filter scenario and compares Mini RxJava with RxJava.
   */
  @Test
  void shouldCollectMetricsForMapFilterScenario() {
    MetricsResult<Integer> miniRxResult =
        measureMiniRx(
            () ->
    ...
    MetricsResult<Integer> rxJavaResult =
        measureRxJava(
            () ->
  ...
  /** Collects metrics for a synchronous flatMap scenario and compares Mini RxJava with RxJava. */
  @Test
   ...
   * @param <T> the item type
   */
  private <T> void assertFunctionalEquality(
      MetricsResult<T> miniRxResult, MetricsResult<T> rxJavaResult) {
   ...
   * @param <T> the item type
   */
  private <T> void printMetrics(
      String scenarioName, MetricsResult<T> miniRxResult, MetricsResult<T> rxJavaResult) {
    System.out.println();
   ...
   * @param <T> the item type
   */
  private record MetricsResult<T>(
      List<T> items, boolean completed, Throwable error, long durationNanos) {}
}

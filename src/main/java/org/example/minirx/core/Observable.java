 ...
 *
 * <p>An {@code Observable} keeps the subscription logic instead of storing items in advance. When a
 * subscriber appears, that logic is executed and pushes signals through an {@link Emitter}.
 *
...
public class Observable<T> {
   ...
   *
   * <p>The resulting stream is completed only after the outer source and all currently active inner
          ...
          Observable.this.subscribe(
              new Observer<>() {
                ...
                public void onNext(T item) {
                  ...
                  try {
                    innerObservable =
                        Objects.requireNonNull(mapper.apply(item), "mapper returned null");
                  } catch (RuntimeException exception) {
                    emitter.onError(exception);
                    return;
                  }
   ...
   *
   * <p>In this implementation each signal is submitted as a separate task. For stable ordering, a
   * single-thread scheduler is the safest option.
   *
